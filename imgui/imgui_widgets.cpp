...
bool ImGui::customTab(const char* text, bool selected, ImVec2 size)
{
    ...
    ImVec2 pos = window->DC.CursorPos;
    ImVec2 soize = { size.x, 20 };
    const ImRect bb(pos, ImVec2(pos.x + soize.x, pos.y + soize.y));

    ImGui::ItemSize(soize, 0);
    ...
    if (selected)
    {
        ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (soize.x - ImGui::CalcTextSize(text).x) / 2, bb.Min.y + (soize.y - ImGui::CalcTextSize(text).y) / 4), hovered ? ImColor(255, 0, 255) : ImColor(255, 0, 255), text);
    }
    else
    {
        ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (soize.x - ImGui::CalcTextSize(text).x) / 2, bb.Min.y + (soize.y - ImGui::CalcTextSize(text).y) / 4), hovered ? ImColor(255, 255, 255) : ImColor(155, 155, 155), text);
    }
    ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y + soize.y - 2), ImVec2(bb.Max.x, bb.Min.y + soize.y), GetColorU32(ImGuiCol_Border), 0, 0);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float preview_width = ((preview_value != NULL)) ? CalcTextSize(preview_value, NULL, true).x : 0.0f;
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : ((flags) ? (arrow_size + preview_width + style.FramePadding.x * 2.0f) : GetContentRegionAvail().x);
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Max.x, frame_bb.Max.y), frame_col, style.FrameRounding);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    RenderFrame(frame_bb.Min, ImVec2(grab_bb.Min.x + 12, frame_bb.Max.y), GetColorU32(ImGuiCol_SliderGrab), false, style.GrabRounding);
    RenderFrameBorder(frame_bb.Min, frame_bb.Max, style.FrameRounding);
